...
def main() -> None:
    ...
    parser.add_argument("--output-dir", default=None, help="default: data/sharadar_full/sep/")
    parser.add_argument(
        "--keep-zip", action="store_true", help="keep the downloaded CSV zip after conversion"
    )
    args = parser.parse_args()

    out_dir = Path(args.output_dir) if args.output_dir else _bulk.FULL_DATA_DIR / "sep"
    _bulk.ingest(
        "SHARADAR/SEP",
        out_dir=out_dir,
        stem="sep",
